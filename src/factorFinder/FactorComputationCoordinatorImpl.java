...
public class FactorComputationCoordinatorImpl
  ...
  public FactorComputationCoordinatorImpl(FactorComputeEngineImpl computeEngine,
      DataStoreServiceBlockingStub dataStoreStub) {
    this.computeEngine = computeEngine;
    this.dataStoreStub = dataStoreStub;
    this.executor =
        Executors.newFixedThreadPool(2); // Adjust thread pool size as needed
  }
    ...
    try {
      ...
      if (dataStoreStub != null) {
        ...
        if (!"success".equals(readResponse.getStatus())) {
          System.err.println(
              "Error reading input file: " + readResponse.getErrorMessage());
          return FactorComputeResult.FAILURE;
        }
        inputData =
            readResponse
                .getDataList(); // set inputData equal to the external inputData
      } else if (dataStore != null) {
        // Using the local FactorDataStoreImpl to read data
        FileInputConfig inputConfig =
            new FileInputConfig(request.getInputConfig().getFilePath());
        inputData = dataStore.read(
            inputConfig); // set inputData equal to the internal inputData
      } else {
      ...
      for (Integer number : inputData) {
        Callable<List<String>> task =
            () -> computeEngine.getFormattedFactors(number);
      ...
      DataStoreWriteRequest writeRequest =
          DataStoreWriteRequest.newBuilder()
              ...
              .build();
      // Write output data to the external dataStore Write Response, using the
      // external DataStoreServiceImpl write method
      if (dataStoreStub != null) {
        // Using gRPC stub to write data
        DataStoreWriteResponse writeResponse =
            dataStoreStub.write(writeRequest);
        if (!"success".equals(writeResponse.getStatus())) {
          System.err.println(
              "Error writing output file: " + writeResponse.getErrorMessage());
          return FactorComputeResult.FAILURE;
        }
      } else if (dataStore != null) {
        ...
        WriteResult writeResult =
            dataStore.appendSingleResult(outputConfig, formattedFactors);
